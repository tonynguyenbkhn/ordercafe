...
define('TWMP_REVENUE_SHIFTS_VERSION', '0.1.15');
...
function twmp_revenue_shifts_create_tables()
{
    ...
    $sql = "CREATE TABLE {$table} (
        ...
        unpaid_sales bigint(20) NOT NULL DEFAULT 0,
        note text NULL,
...
function twmp_revenue_shifts_get_period_totals($entries)
{
    ...
    foreach ((array) $entries as $shift_entries) {
        foreach ((array) $shift_entries as $entry) {
            ...
            $totals['unpaid_sales'] += isset($entry['unpaid_sales']) ? (int) $entry['unpaid_sales'] : 0;
...
function twmp_revenue_shifts_render_shortcode()
{
    ...
    $rows = [
        ...
        'unpaid_sales'        => __('Chưa thanh toán', 'twmp-revenue-shifts'),
                                        ...
                                        <?php
                                            ...
                                            <td class="is-today" data-date="<?php echo esc_attr($date); ?>" data-shift="<?php echo esc_attr($shift); ?>">
                                                ...
                                                if ('staff_user_id' === $field) {
                                                    ...
                                                    $is_readonly_money = in_array($field, ['cash_sales', 'bank_transfer_sales', 'unpaid_sales', 'failed_sales'], true);
...
function twmp_revenue_shifts_export_month()
{
    ...
    fputcsv($output, []);
    fputcsv($output, ['Ngày', 'Ca', 'Người chốt', 'Tiền mặt', 'Chuyển khoản', 'Chưa thanh toán', 'Đơn lỗi', 'Chi', 'Doanh thu']);
    ...
    for ($date = $range['start']; $date <= $range['end']; $date = $date->modify('+1 day')) {
        ...
        foreach ($shifts as $shift_key => $shift_label) {
            ...
            fputcsv($output, [
                ...
                isset($entry['unpaid_sales']) ? (int) $entry['unpaid_sales'] : 0,
...
function twmp_revenue_shifts_rest_import_orders(WP_REST_Request $request)
{
    ...
    foreach (['morning', 'afternoon'] as $shift_key) {
        ...
        $result['entries'][$date][$shift_key]['unpaid_sales'] = isset($result['entries'][$date][$shift_key]['unpaid_sales']) ? $result['entries'][$date][$shift_key]['unpaid_sales'] : 0;
    ...
    $saved = twmp_revenue_shifts_save_entries($branch_id, $month, $result['entries'], ['cash_sales', 'bank_transfer_sales', 'unpaid_sales', 'failed_sales']);
...
{
    return sprintf(
        /* translators: 1: order count, 2: cash total, 3: bank total, 4: unpaid total, 5: failed total, 6: skipped order count */
        __('Đã lấy %1$d đơn. Tiền mặt: %2$s. Chuyển khoản: %3$s. Chưa thanh toán: %4$s. Đơn lỗi: %5$s. Bỏ qua: %6$d đơn.', 'twmp-revenue-shifts'),
        ...
        twmp_revenue_shifts_format_money(isset($summary['unpaid_total']) ? $summary['unpaid_total'] : 0),
...
function twmp_revenue_shifts_import_orders($branch_id, $month, $date = '', $exclude_order_ids = array(), $staff_user_id = 0)
{
    ...
    $unpaid_methods = apply_filters('twmp_revenue_shifts_unpaid_payment_methods', ['cheque']);
    $entries = [];
    $summary = [
        'orders_count' => 0,
        'cash_orders_count' => 0,
    ...
    foreach ($orders as $order) {
        ...
        if (in_array($payment_method, $unpaid_methods, true)) {
            $entries[$business_date][$shift_key]['unpaid_sales'] += $total;
            $summary['unpaid_orders_count']++;
            $summary['unpaid_total'] += $total;
            continue;
        }
...
function twmp_revenue_shifts_sync_month_from_order($order, $exclude_current_order = true)
{
    ...
    twmp_revenue_shifts_save_entries($branch_id, $month, $result['entries'], ['cash_sales', 'bank_transfer_sales', 'unpaid_sales', 'failed_sales']);
    ...
    foreach ($entries as $date => $shift_entries) {
        ...
        foreach ($shift_entries as $shift_key => $entry) {
            ...
            $data = [
                ...
                'unpaid_sales'         => isset($existing['unpaid_sales']) ? (int) $existing['unpaid_sales'] : 0,
                'status'               => 'draft',
                'updated_at'           => $now,
            ];
