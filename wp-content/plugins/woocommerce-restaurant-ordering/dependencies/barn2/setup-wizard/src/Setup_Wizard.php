...
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Setup_Wizard\Interfaces\Bootable;
...
class Setup_Wizard implements Bootable, JsonSerializable
{
    ...
    public function register_admin_page()
    {
        ...
        $page_title = \sprintf(__('%s setup wizard', 'woocommerce-restaurant-ordering'), $this->plugin->get_name());
    ...
    public function load_wizard_restart_assets()
    {
		...
		<script>
			jQuery( '.barn2-wiz-restart-btn' ).on( 'click', function( e ) {
				/* translators: %s: The name of the plugin. */
				return confirm( '<?php 
        echo \esc_html(\sprintf(__('Warning: This will overwrite your existing settings for %s. Are you sure you want to continue?', 'woocommerce-restaurant-ordering'), $this->plugin->get_name()));
        ?>' );
			});
		</script>
		<?php 
    ...
    public function add_restart_btn()
    {
		...
		<div class="barn2-setup-wizard-restart">
			<hr>
			<h3><?php 
        esc_html_e('Setup wizard', 'woocommerce-restaurant-ordering');
        ?></h3>
			<p><?php 
        esc_html_e('If you need to access the setup wizard again, please click on the button below.', 'woocommerce-restaurant-ordering');
        ?></p>
			<a href="<?php 
        echo \esc_url($url);
    ...
    public function add_restart_link(string $wc_section_id, string $title_option_id)
    {
        \add_filter('barn2_plugin_settings_help_links', function ($links, $plugin) use($title_option_id) {
            ...
            $url = \add_query_arg(['page' => $this->get_slug()], \admin_url('admin.php'));
            $links['setup_wizard'] = ['url' => \esc_url($url), 'label' => esc_html__('Setup wizard', 'woocommerce-restaurant-ordering'), 'class' => 'barn2-wiz-restart-btn'];
            return $links;
        }, 10, 2);
