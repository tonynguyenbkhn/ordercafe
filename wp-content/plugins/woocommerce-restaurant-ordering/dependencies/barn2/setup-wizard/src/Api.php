 ...
 */
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Setup_Wizard;

use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Setup_Wizard\Interfaces\Deferrable;
...
class Api implements JsonSerializable
{
    ...
    public function can_view_steps($request)
    {
        ...
        if (\is_user_logged_in()) {
            if (!\current_user_can('manage_options')) {
                return new \WP_Error('rest_forbidden', __('You do not have permission to view the setup wizard.', 'woocommerce-restaurant-ordering'), ['status' => 403]);
            }
            return \true;
        }
        return new \WP_Error('rest_forbidden', __('Authentication required.', 'woocommerce-restaurant-ordering'), ['status' => \rest_authorization_required_code()]);
    }
    /**
    ...
    public function save_fields($request)
    {
        ...
        if (empty($step) || !$step instanceof Step) {
            return self::send_error_response(['message' => __('Could not find the appropriate step.', 'woocommerce-restaurant-ordering')]);
        }
    ...
    public function handle_license($request)
    {
        ...
        if (empty($license_key)) {
            return self::send_error_response(['message' => __('Please enter a license key.', 'woocommerce-restaurant-ordering')]);
        }
        if (!\in_array($action, $allowed_actions, \true)) {
            return self::send_error_response(['message' => __('Invalid action requested.', 'woocommerce-restaurant-ordering')]);
        }
        switch ($action) {
            case 'activate':
