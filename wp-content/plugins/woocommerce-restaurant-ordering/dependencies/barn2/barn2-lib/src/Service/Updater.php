...
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Service;
...
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Conditional;
/**
...
abstract class Updater implements Standard_Service, Registerable, Plugin_Activation_Listener
{
    ...
    private function get_translated_options()
    {
        ...
        $translated_options['needs_update_db_notice']['title'] = \sprintf(__('%1$s database update required','woocommerce-restaurant-ordering' ), $this->plugin->get_name());
        /* translators: %1$s: plugin name */
        $translated_options['needs_update_db_notice']['message'] = \sprintf(__('<p>%1$s has been updated! To keep things running smoothly, we have to update your database to the newest version. The database update process runs in the background and may take a little while, so please be patient.</p>','woocommerce-restaurant-ordering' ), $this->plugin->get_name());
        /* translators: %1$s: plugin name */
        $translated_options['update_db_complete_notice']['title'] = \sprintf(__('%1$s database update done','woocommerce-restaurant-ordering' ), $this->plugin->get_name());
        /* translators: %1$s: plugin name */
        $translated_options['update_db_complete_notice']['message'] = \sprintf(__('<p>%1$s database update complete. Thank you for updating to the latest version!</p>','woocommerce-restaurant-ordering' ), $this->plugin->get_name());
    ...
    {
        if (!\current_user_can('manage_options')) {
            \wp_die(esc_html__('You do not have sufficient permissions to perform this action.','woocommerce-restaurant-ordering' ));
        }
        // If already locked, stop.
        if ($this->is_upgrade_locked()) {
            \wp_die(esc_html__('An upgrade is already in progress. Please wait until it completes.','woocommerce-restaurant-ordering' ));
        }
