...
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib;
...
class Util
{
    ...
    {
        if (empty($link_text)) {
            $link_text = __('Read more', 'woocommerce-restaurant-ordering');
        }
    ...
    public static function install_bonus_plugins($bonus_plugins)
    {
        ...
        foreach ($bonus_plugins as $plugin) {
                ...
                if (\is_wp_error($skin->result)) {
                    ...
                    if ($skin->get_errors()->get_error_code()) {
                        ...
                        if (\is_null($result)) {
                            ...
                            $error_message = __('Unable to connect to the filesystem. Please confirm your credentials.', 'woocommerce-restaurant-ordering');
            ...
            if ($plugin && \current_user_can('activate_plugin', $plugin)) {
                ...
                $results[$name] = new WP_Error('bonus_download_no_activation_permission', esc_html__('You don\'t have permission to activate the plugin.', 'woocommerce-restaurant-ordering'));
                continue;
            }
            $results[$name] = \true;
        }
