...
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Admin;
...
class Settings_API_Helper implements Registerable, Conditional
{
    ...
    private static function field_description($args)
    {
        if (!empty($args['desc'])) {
            ...
            if ($is_page_select) {
                $args['desc'] .= ' <a href="' . \esc_url(\get_permalink($args['show_page'])) . '" target="_blank">' . esc_html__('[View page]', 'woocommerce-restaurant-ordering') . '</a>';
            }
    ...
    public static function settings_field_color($args)
    {
        ...
        esc_attr_e($args['input_class'],'woocommerce-restaurant-ordering' );
        ?>">
			<input
				type="text"
				name="<?php 
    ...
    public static function settings_field_color_size($args)
    {
        ...
        $size_placeholder = !empty($args['placeholder']) ? $args['placeholder'] : __('Size', 'woocommerce-restaurant-ordering');
        if (empty($args['custom_attributes'])) {
