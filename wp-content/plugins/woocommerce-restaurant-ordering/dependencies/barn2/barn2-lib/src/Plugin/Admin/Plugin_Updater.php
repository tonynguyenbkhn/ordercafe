...
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Plugin\Admin;

use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Plugin\License\License_API;
...
class Plugin_Updater implements Registerable, Core_Service
{
    ...
    public function update_available_notice($plugin_data, $response)
    {
        ...
        if (empty($response->package)) {
            ...
            \printf(' <em>%s</em>', \sprintf(
                ...
                __('Activate %1$syour license key%2$s to enable updates.', 'woocommerce-restaurant-ordering'),
                $settings_link_open,
                $settings_link_close
            ));
    ...
    function auto_update_setting_html($html, $plugin_file)
    {
        ...
        if (!$this->plugin->get_license()->is_valid() || !\apply_filters('barn2_plugin_allow_automatic_update', \true, $this->plugin)) {
            $html = \sprintf('<em>%s</em>', __('Auto-updates unavailable.', 'woocommerce-restaurant-ordering'));
        }
        return $html;
    }
    /**
