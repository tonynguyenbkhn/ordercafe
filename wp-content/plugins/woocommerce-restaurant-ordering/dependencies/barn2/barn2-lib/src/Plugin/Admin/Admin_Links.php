...
class Admin_Links implements Registerable, Standard_Service
{
    ...
    public function add_settings_link($links)
    {
        ...
        \array_unshift($links, \sprintf('<a href="%1$s">%2$s</a>', \esc_url($settings_url), __('Settings', 'woocommerce-restaurant-ordering')));
        return $links;
    }
    public function add_documentation_link($links, $file)
    {
        ...
        $row_meta = ['docs' => \sprintf(
            ...
            \esc_attr(\sprintf(__('View %s documentation', 'woocommerce-restaurant-ordering'), $this->plugin->get_name())),
            esc_html__('Docs', 'woocommerce-restaurant-ordering')
        )];
