...
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Plugin\License\Admin;

use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Admin\Settings_API_Helper;
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Plugin\License\License;
...
class License_Key_Setting implements Registerable, License_Setting, Core_Service
{
    ...
    public function process_license_action()
    {
        if ($this->is_license_action(self::ACTIVATE_KEY)) {
            ...
            if (isset($license_setting['license'])) {
                ...
                $this->add_settings_message(__('License key successfully activated.', 'woocommerce-restaurant-ordering'), __('There was an error activating your license key.', 'woocommerce-restaurant-ordering'), $activated);
            }
        } elseif ($this->is_license_action(self::DEACTIVATE_KEY)) {
            $deactivated = $this->license->deactivate();
            $this->add_settings_message(__('License key successfully deactivated.', 'woocommerce-restaurant-ordering'), __('There was an error deactivating your license key, please try again.', 'woocommerce-restaurant-ordering'), $deactivated);
        } elseif ($this->is_license_action(self::CHECK_KEY)) {
            $this->license->refresh();
            $this->add_settings_message(__('The license key looks good!', 'woocommerce-restaurant-ordering'), __('There\'s a problem with your license key.', 'woocommerce-restaurant-ordering'), $this->license->is_active());
        }
    }
    ...
    {
        $setting = ['title' => __('License key', 'woocommerce-restaurant-ordering'), 'type' => 'text', 'id' => $this->get_license_setting_name() . '[license]', 'desc' => $this->get_license_description(), 'class' => 'regular-text barn2-license-key'];
        if ($this->is_woocommerce) {
            $setting['desc_tip'] = __('The licence key is contained in your order confirmation email.', 'woocommerce-restaurant-ordering');
        } elseif ($this->is_edd) {
            // EDD uses title case for setting names, so let's keep things consistent.
            $setting['title'] = __('License Key', 'woocommerce-restaurant-ordering');
    ...
    {
        $buttons = ['check' => $this->license_action_button(self::CHECK_KEY, __('Check', 'woocommerce-restaurant-ordering')), 'activate' => $this->license_action_button(self::ACTIVATE_KEY, __('Activate', 'woocommerce-restaurant-ordering')), 'deactivate' => $this->license_action_button(self::DEACTIVATE_KEY, __('Deactivate', 'woocommerce-restaurant-ordering'))];
        ...
        if ($this->license->is_active()) {
            ...
            if ($this->license->is_inactive() && $this->is_license_action('deactivate_key')) {
                ...
                $message = __('License key deactivated.', 'woocommerce-restaurant-ordering');
            } else {
