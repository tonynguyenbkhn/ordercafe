...
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Plugin;
...
class Requirements implements Core_Service
{
        ...
        if (\version_compare(\PHP_VERSION, $required_php, '<')) {
            if (\defined('WP_CLI') && \WP_CLI) {
                \wp_die(\sprintf(
                    ...
                    __('%1$s requires PHP %2$s or greater. Please update the PHP version used by your web server.', 'woocommerce-restaurant-ordering'),
        ...
        if (\version_compare($wp_version, $required_wp, '<')) {
            if (\defined('WP_CLI') && \WP_CLI) {
                \wp_die(\sprintf(
                    ...
                    __('%1$s requires WordPress %2$s or greater. Please update your WordPress installation.', 'woocommerce-restaurant-ordering'),
                    $this->plugin->get_name(),
                    $required_wp
                ));
            }
            if (\is_admin()) {
                if ($add_notice) {
                    ...
                    $this->plugin->notices()->add_error_notice($this->plugin->get_slug() . '_invalid_wp_version', '', \sprintf(
                        ...
                        __('%1$s requires WordPress %4$s or greater. Please %2$supdate%3$s your WordPress installation.','woocommerce-restaurant-ordering' ),
                        $this->plugin->get_name(),
    ...
    public final function check_wc(bool $add_notice = \true) : bool
    {
        if (!\class_exists('WooCommerce')) {
            if (\defined('WP_CLI') && \WP_CLI) {
                ...
                \wp_die(\sprintf(__('%1$s requires WooCommerce to be installed and active.', 'woocommerce-restaurant-ordering'), $this->plugin->get_name()));
            }
            if (\is_admin()) {
                if ($add_notice) {
                    $this->plugin->notices()->add_error_notice($this->plugin->get_slug() . '_woocommerce_missing', '', \sprintf(
                        ...
                        __('%1$s requires %2$s to be installed and active.%3$s', 'woocommerce-restaurant-ordering'),
                        $this->plugin->get_name(),
        ...
        if (\version_compare($woocommerce->version, $required_wc, '<')) {
            if (\defined('WP_CLI') && \WP_CLI) {
                ...
                \wp_die(\sprintf(__('%1$s requires WooCommerce %2$s or greater. Please update your WooCommerce setup first.','woocommerce-restaurant-ordering' ), $this->plugin->get_name(), $required_wc));
            }
            if (\is_admin()) {
                if ($add_notice) {
                    $this->plugin->notices()->add_error_notice($this->plugin->get_slug() . '_invalid_woocommerce_version', '', \sprintf(
                        ...
                        __('%1$s requires %2$s %3$s or greater. Please %4$s your WooCommerce setup first.','woocommerce-restaurant-ordering' ),
                        $this->plugin->get_name(),
                        Util::get_plugin_link('WooCommerce', 'woocommerce'),
                        $required_wc,
    ...
    public final function check_edd($add_notice = \true) : bool
    {
        if (!\class_exists('Easy_Digital_Downloads')) {
            if (\defined('WP_CLI') && \WP_CLI) {
                /* translators: %1$s: Plugin name. */
                \wp_die(\sprintf(__('%1$s requires Easy Digital Downloads to be installed and active.','woocommerce-restaurant-ordering' ), $this->plugin->get_name()));
            }
            if (\is_admin()) {
                if ($add_notice) {
