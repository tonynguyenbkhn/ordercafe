...
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Plugin\Admin\Notice_Provider;
...
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Service\Service_Provider;
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Util;
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\WooCommerce\Compatibility as WooCommerce_Compatibility;
use Exception;
/**
    ...
    public final function before_activate($network_wide)
    {
        ...
        if (!\get_option($this->get_basename() . '_plugin_activation')) {
            try {
                ...
                $this->notices()->add_error_notice($this->get_slug() . '_activation_error', $this->get_name(), \sprintf(__('%1$s could not complete its first activation because the following error occurred:<br>%2$s', 'woocommerce-restaurant-ordering'), $this->get_name(), $e->getMessage()), ['capability' => 'install_plugins', 'screens' => ['plugins']]);
            }
        }
