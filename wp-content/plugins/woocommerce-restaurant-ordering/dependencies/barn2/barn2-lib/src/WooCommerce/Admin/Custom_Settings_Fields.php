...
namespace Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\WooCommerce\Admin;
...
use Barn2\Plugin\WC_Restaurant_Ordering\Dependencies\Lib\Admin\Settings_Util;
...
class Custom_Settings_Fields implements Registerable
{
    ...
    public function color_size_field($value)
    {
        ...
        $custom_attributes = Settings_Util::get_custom_attributes($value);
        $field_description = WC_Admin_Settings::get_field_description($value);
        $size_placeholder = !empty($value['placeholder']) ? $value['placeholder'] : __('Size', 'woocommerce-restaurant-ordering');
